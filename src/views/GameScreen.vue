...
<script>
  const rockSfx = require('../assets/sfx/rock.mp3')
  const paperSfx = require('../assets/sfx/paper.mp3')
  const scissorSfx = require('../assets/sfx/scissor.mp3')
  ...
  export default {
    ...
    methods: {
      playSfx(rockOrPaperOrScissors) {
        if (rockOrPaperOrScissors === 1) {
          let rock = new Audio(rockSfx)
          rock.play()
        } else if (rockOrPaperOrScissors === 2) {
          let paper = new Audio(paperSfx)
          paper.play()
        } else if (rockOrPaperOrScissors === 3) {
          let scissor = new Audio(scissorSfx)
          scissor.play()
        } 
      },
      ...
      async playerInput(rockOrPaperOrScissors) {
        await this.playSfx(rockOrPaperOrScissors)
        if (this.isCountDownRun === false) await this.countDownTimer()
