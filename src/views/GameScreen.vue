<template>
  <v-card class="rounded-xl" elevation="8" max-width="400">
    ...
    <v-card-text class="text-center">
      ...
      <v-btn @click="redirectToMainMenuPage(); btnSfx()" :disabled="this.isCountDownRun" class="mt-n7" x-small color="secondary">
        Back To Main Menu
      </v-btn>
    </v-card-text>
    ...
    <v-dialog v-model="dialogFinalResult" persistent width="500">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="black" text @click="resetGame(); btnSfx()">
            Try again
          </v-btn>
          <v-btn color="black" text @click="redirectToMainMenuPage(); btnSfx();">
            Back To Main Menu
          </v-btn>
        </v-card-actions>
      </v-card>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      playerInputSfx(rockOrPaperOrScissors) {
        if (rockOrPaperOrScissors === 1) {
          this.playSfx(require('../assets/sfx/rock.mp3'))
        } else if (rockOrPaperOrScissors === 2) {
          this.playSfx(require('../assets/sfx/paper.mp3'))
        } else if (rockOrPaperOrScissors === 3) {
          this.playSfx(require('../assets/sfx/scissor.mp3'))
        }
      },
      btnSfx() {
        this.playSfx(require('../assets/sfx/btn.mp3'))
      },
      showConfetti() {
        let x = 0;
      ...
      async playerInput(rockOrPaperOrScissors) {
        await this.playerInputSfx(rockOrPaperOrScissors)
      ...
      countDownTimer() {
        ...
        if (this.countDown > 0) {
          let timer = setInterval(() => {
            ...
            if (vm.countDown === 0 && vm.scorePlayer === vm.scoreEnemy) {
              clearInterval(timer)
              this.dialogDraw()
            ...
            } else if (vm.countDown === 0 && vm.scorePlayer < 20 && vm.scoreEnemy < 20) {
              clearInterval(timer)
